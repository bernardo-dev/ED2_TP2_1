...
void iniciaHeap(Heap *heap) { heap->n = heap->marcados = 0; }
...
void heapRefaz(Heap *heap, short esq, short dir, Metrica *metrica) {
  ...
  while (j <= dir) {
    if (j < dir) {
      ...
      if (!(heap->itens[j].marcado == false &&
            heap->itens[j + 1].marcado == true)) {
        /*
            Os fluxos condicionais seguintes servem para indicar os casos em que
           o filho mais a direita eh menor que o filho mais a esquerda
        */
        metrica->n_comparacao++;
        if ((heap->itens[j].marcado == true &&
             heap->itens[j + 1].marcado == false) ||
...
void abrirFitas(FILE *fitas_entrada[N_FITAS_ENTRADA],
  ...
  // --- Mudanca ---
  // Como o numero de fitas de saida aqui é maior que 1, repete o mesmo processo
  // da fita de entrada
  // ----- Codigo f1 -------
