...
// void converterFitasParaTexto(FILE *fitas_entrada[N_FITAS_ENTRADA], Lista blocos_por_fita[N_FITAS_ENTRADA])
// {
//     Registro buffer;
...
//             printf("BLOCO %u | QTDE DE ITENS : %u\n", c_blocos, blocos_por_fita[i].cabeca->proximo->qtde_itens_bloco);

//             // Varre o bloco
//             while(c_itens < blocos_por_fita[i].cabeca->proximo->qtde_itens_bloco)
//             {
...
void intercalacaoBalanceada2f(FILE *fitas_entrada[N_FITAS_ENTRADA], FILE *fitas_saida[N_FITAS_SAIDA], Lista blocos_por_fita[N_FITAS_ENTRADA], Lista blocos_fita_saida[N_FITAS_SAIDA], unsigned short indice_fita_saida, Metrica *metricas)
{
    ...
    ItemHeap memoria[TAM_MEMORIA_PRINCIPAL];     // Guarda os registros das fitas. Cada posicao eh um indice de uma fita.
    ...
    // ---------- Mudanca-------------
    //Como agr existe mais de uma fita de saida, precisa-se de um vetor para contabilizar os itens por bloco
    // ----------- F1 fitas ----------
    itens_por_bloco = 0;
    ...
    while(marcados != TAM_MEMORIA_PRINCIPAL)
    {
        ...
        fwrite(&memoria[k].registro, sizeof(Registro), 1, fitas_saida[indice_fita_saida]);
        metricas->n_escrita++;
        itens_por_bloco++;
    ...
    if(itens_por_bloco != 0){
        metricas->n_comparacao++;
        insereLista(&blocos_fita_saida[indice_fita_saida], itens_por_bloco, metricas);
    }
}

FILE* doisFfitas(FILE *arquivo_binario, unsigned int qtde_registros, Metrica *metricas)
{   
    clock_t inicio;
    clock_t fim;
    ...
    Lista blocos_fitas_entrada[N_FITAS_ENTRADA];    // Cada posicao do vetor eh uma lista encadeada de blocos presentes em cada fita.
    Lista blocos_fitas_saida[N_FITAS_SAIDA];                        // Lista encadeada de blocos que a fita de saida ira possuir.
    unsigned short indice_fita_saida;

    // Cria os arquivos necessarios e os abre.
    abrirFitas(fitas_entrada, fitas_saida, metricas);

    iniciaListas(blocos_fitas_entrada, blocos_fitas_saida, metricas);

    rewind(arquivo_binario);

    geracaoBlocosOrdenados(arquivo_binario, qtde_registros, fitas_entrada, blocos_fitas_entrada, metricas);
    ...
    {   
        ...
        while(!todasAsFitasDeEntradaEstaoVazias(blocos_fitas_entrada, metricas)) {
            intercalacaoBalanceada2f(fitas_entrada, fitas_saida, blocos_fitas_entrada, blocos_fitas_saida, indice_fita_saida, metricas);
            indice_fita_saida++;
        ...
        //redistribuirBlocos(fitas_entrada, fitas_saida, blocos_fitas_entrada, &blocos_fita_saida);
        saida_entrada(fitas_entrada, fitas_saida, blocos_fitas_entrada, blocos_fitas_saida);
    }
